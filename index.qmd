...
::: {style="margin-top: 0.8em; font-size: 0.85em; color: #94a3b8;"}
...
A single neuron that learns from examples by adjusting weights $w_i$.

Inspired by McCulloch-Pitts neurons (1943) and Hebbian learning.

::: {.fragment}
**1969 — Minsky & Papert:** the perceptron cannot solve XOR.
Neural networks enter a decade-long winter.
:::

::: {.fragment}
**1986 — Rumelhart, Hinton, Williams:** backpropagation.
Multi-layer networks can learn any function.
:::
...
This is the origin of gradient-based learning.
Every modern neural network still follows this principle.
:::
...
::: notes
Rosenblatt built the Mark 1 Perceptron at Cornell in 1958 — a physical hardware device that learned to recognise patterns from photographs. It made the front page of the New York Times.

The Minsky-Papert result was a setback, but in retrospect the issue was simply that one layer isn't enough. Two layers can solve XOR. Backpropagation made multi-layer training tractable.
:::
...
fig.update_layout(
    height=460, autosize=True, paper_bgcolor="#0d1b2a", plot_bgcolor="#0d1b2a",
...
::: {.fragment style="margin-top: 0.8em; font-size: 0.88em;"}
Physics concepts still active in modern ML:

- Renormalisation group ↔ deep hierarchies
- Path integrals ↔ diffusion models
:::
...
fig.update_layout(
    height=490, autosize=True, paper_bgcolor="#0d1b2a", plot_bgcolor="#111f30",
...
- Clinical machines are designed to be safe → faults are **rare**
...
fig.update_layout(height=440, autosize=True, paper_bgcolor="#0d1b2a", plot_bgcolor="#111f30",
...
<iframe src="https://nbviewer.org/github/aghribi/cas2026-ai-medical-accelerators/blob/main/notebooks/01_fault_detection/notebook.ipynb"
...
- Finds a good solution in **~50 evaluations**
- Model-free — works on the real machine
- GP surrogate builds up a map of the objective
:::
...
::: {.metric style="margin-top: 0.8em;"}
σ_x = 1.00 mm (input)
σ_x + σ_y = 4.83 mm (no quads)
:::
...
fig.update_layout(height=390, autosize=True,
...
<iframe src="https://nbviewer.org/github/aghribi/cas2026-ai-medical-accelerators/blob/main/notebooks/02_beam_tuning/notebook.ipynb"
        width="100%" height="530px" frameborder="0"
        style="border-radius:6px; border:1px solid #22d3ee; background:#0d1b2a;">
</iframe>
...
1. Run the slow tracking code offline for thousands of random inputs
2. Train a neural network on those input-output pairs
3. Call the NN instead of the simulator during optimisation

::: {.key-result style="margin-top: 0.8em;"}
Typical speedup: **10³ – 10⁵×**

Typical error: **< 1%** in the training domain
:::
...
fig.update_layout(
    ...
    xaxis=dict(gridcolor="#1e3048"),
    height=420, autosize=True, paper_bgcolor="#0d1b2a", plot_bgcolor="#111f30",
...
<iframe src="https://nbviewer.org/github/aghribi/cas2026-ai-medical-accelerators/blob/main/notebooks/03_surrogate_model/notebook.ipynb"
        width="100%" height="530px" frameborder="0"
        style="border-radius:6px; border:1px solid #22d3ee; background:#0d1b2a;">
</iframe>
...
- Adaptive QA — detect drift, flag recalibration
- Cross-facility transfer — pre-train at research machine, fine-tune at clinical site
...
Faults are rare by design — transfer learning from research to clinical machines is not yet solved.
:::

::: {.highlight-box}
**Cross-facility transfer**

A model trained at Centre A rarely works at Centre B without retraining — domain adaptation is needed.
:::
...
EU AI Act classifies control-loop AI as high risk — auditable, explainable decisions are required.
:::

::: {.highlight-box}
**Real-time integration**

Embedding PyTorch models into proprietary clinical control systems (Varian, Elekta) is non-trivial.
:::
...
These are active research problems — not blockers. AI is already delivering value in clinical operations today.
:::
