...
<div class="concept-callout-label"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;vertical-align:-0.15em;"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg> Concept — Training epoch &amp; ROC-AUC</div>
<div style="display:flex; gap:1.6em;">
<div style="flex:1; border-right:1px solid rgba(251,191,36,0.25); padding-right:1.2em;">
<strong>Epoch</strong> — one complete pass over the entire training dataset.<br>
1 epoch = dataset_size / batch_size iterations (gradient steps).<br><br>
Too few epochs → underfitting.<br>
Too many → overfitting (memorises noise).<br>
<span style="color:#94a3b8; font-size:0.90em;">Stop when validation loss starts rising while training loss still falls.</span>
</div>
<div style="flex:1; padding-left:0.4em;">
<strong>ROC-AUC</strong> — sweep the decision threshold 0→1, plot TPR vs FPR at each threshold.<br>
Area under that curve = AUC.<br><br>
AUC = 0.5 → random classifier<br>
AUC = 1.0 → perfect separation<br>
<span style="color:#94a3b8; font-size:0.90em;">Threshold-independent → ideal for rare-event detection (SRF faults &lt; 0.1 %).</span>
</div>
</div>
</div>
</div>
...
<div class="fragment slide-callout-overlay">
<div class="concept-callout-box">
<div class="concept-callout-label"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;vertical-align:-0.15em;"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg> Concept — Bayesian inference &amp; prior</span>
<div style="text-align:center; font-size:1.05em; margin-bottom:0.55em; color:#e2e8f0;">
P(θ | data) &nbsp;∝&nbsp; P(data | θ) &nbsp;×&nbsp; P(θ)
</div>
<div style="display:flex; gap:1.6em; font-size:0.93em;">
<div style="flex:1; border-right:1px solid rgba(251,191,36,0.25); padding-right:1.2em;">
<strong>Prior</strong> P(θ): your belief <em>before</em> seeing data.<br>
Encodes physics knowledge — e.g. "the function is smooth", "noise ≈ 1 %".<br><br>
<strong>Posterior</strong> P(θ|data): updated belief <em>after</em> data.<br>
This is what BO uses as its surrogate model.
</div>
<div style="flex:1; padding-left:0.4em;">
<strong>vs. Frequentist:</strong> only data has probability; hypotheses do not. Bayesian methods assign probabilities to hypotheses directly — more natural for decision-making under uncertainty.<br><br>
<span style="color:#94a3b8; font-size:0.92em;">In BO: a distribution over possible objective functions, not one guess. Explores where uncertain, exploits where confident.</span>
</div>
</div>
</div>
</div>
...
<div class="fragment slide-callout-overlay">
<div class="concept-callout-box">
<div class="concept-callout-label"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;vertical-align:-0.15em;"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg> Concept — Gaussian process &amp; kernel</div>
<div style="display:flex; gap:1.6em;">
<div style="flex:1.1; border-right:1px solid rgba(251,191,36,0.25); padding-right:1.2em;">
<strong>Gaussian process</strong> — a distribution over functions.<br>
Defined by a mean m(x) and a <strong>kernel</strong> k(x,x').<br>
At any finite set of inputs → joint Gaussian.<br>
Posterior is closed-form: mean + variance.<br>
<span style="color:#94a3b8; font-size:0.90em;">Far from data → wide uncertainty band → explore there.<br>
Near data → narrow band → exploit there.</span>
</div>
<div style="flex:0.9; padding-left:0.4em; font-size:0.92em;">
<strong>Kernel = prior on smoothness</strong><br><br>
<strong>RBF / SE</strong>: very smooth, ∞-differentiable<br>
<strong>Matérn 5/2</strong>: physical systems default<br>
<strong>Matérn 3/2</strong>: rougher, noisier data<br>
<strong>Periodic</strong>: cyclic signals (RF phase)<br>
<strong>Linear</strong>: Bayesian linear regression<br><br>
<span style="color:#94a3b8; font-size:0.90em;">For beam tuning: Matérn 5/2 is the practical default.</span>
</div>
</div>
</div>
</div>
...
<div class="fragment slide-callout-overlay">
<div class="concept-callout-box">
<div class="concept-callout-label"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;vertical-align:-0.15em;"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg> Concept — Uncertainty quantification (UQ)</div>
<div style="display:flex; gap:1.6em;">
<div style="flex:1; border-right:1px solid rgba(251,191,36,0.25); padding-right:1.2em;">
<strong>Epistemic</strong> — model uncertainty.<br>
Caused by insufficient training data. <em>Reducible</em>: add more samples.<br>
<span style="color:#94a3b8; font-size:0.90em;">"We don't know because we haven't looked enough."</span><br><br>
<strong>Aleatoric</strong> — measurement noise.<br>
Inherent randomness in the data. <em>Irreducible</em>.<br>
<span style="color:#94a3b8; font-size:0.90em;">"The machine itself is stochastic."</span>
</div>
<div style="flex:1; padding-left:0.4em; font-size:0.93em;">
<strong>Methods:</strong><br>
<strong>Deep ensembles</strong> — spread of N independent models.<br>
<strong>MC Dropout</strong> — stochastic forward passes at inference.<br>
<strong>Gaussian processes</strong> — closed-form posterior variance.<br>
<strong>Conformal prediction</strong> — coverage guarantees, no assumptions.<br><br>
<span style="color:#94a3b8; font-size:0.92em;">Knowing <em>when the model is uncertain</em> is as important as the prediction itself for safety-critical decisions.</span>
</div>
</div>
</div>
</div>
...
<div style="display:flex; gap:1.6em;">
<div style="flex:1.1; border-right:1px solid rgba(251,191,36,0.25); padding-right:1.2em;">
<strong>SHAP</strong> — assigns each input a numerical contribution to the prediction.
Grounded in Shapley values from cooperative game theory (1953). Model-agnostic.<br>
<strong>LIME</strong> — fits a local linear model around one specific prediction to explain it.<br>
<strong>Feature importance</strong> — global ranking of which inputs drive most predictions.
</div>
<div style="flex:0.9; padding-left:0.4em; font-size:0.93em;">
<strong>Why it matters here:</strong><br>
EU AI Act (2024) classifies control-loop AI on clinical machines as <em>high-risk</em> — auditable, explainable decisions required.<br><br>
<span style="color:#94a3b8; font-size:0.92em;">In practice: use SHAP to verify the model learned physics, not a data artefact.</span>
</div>
</div>
</div>
</div>
