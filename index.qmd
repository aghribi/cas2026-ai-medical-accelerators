...
<div style="position:absolute; top:0.2em; right:0.4em; z-index:10; display:flex; gap:0.5em;">
...
## Hopfield networks — memory as energy minimisation

:::: {.columns style="margin-top:0.3em;"}
::: {.column width="52%"}

**Analogy with spin glasses** (Ising model):

- Neurons $s_i \in \{-1, +1\}$ — binary, like magnetic spins
- Weights $w_{ij}$ — like spin-coupling constants
- Energy always descends toward a minimum → **stored memories = energy minima**
- A noisy cue evolves toward the nearest stored pattern

::: {style="font-size:0.78em; margin-top:0.5em; line-height:1.5;"}
**Hebbian learning rule** (pattern storage):
$$w_{ij} = \frac{1}{N}\sum_\mu \xi_i^\mu \xi_j^\mu$$
Patterns $\xi^\mu$ are imprinted as valleys in the energy landscape.
:::
...
**Retrieval = gradient descent on E**

Starting from a partial cue, the network rolls downhill
in energy space to the nearest stored memory —
...
Derived via replica methods from statistical mechanics —
not a computer science result.
:::
...
::: {.slide-ref}
Hopfield, J.J. (1982). Neural networks and physical systems with emergent collective computational abilities. *PNAS* 79, 2554–2558. · Amit, Gutfreund & Sompolinsky (1985). *PRL* 55, 1530.
:::

::: notes
The capacity result 0.14N comes from Amit, Gutfreund and Sompolinsky (1985) using the replica trick from spin glass theory — pure statistical mechanics. The overlap with Ising models, partition functions and free energy is not a metaphor: it is the same mathematics.
:::
...
::: {style="margin-top:0.2em; font-size:0.88em;"}
...
Before any model, **the data must be trustworthy**.

::: {.highlight-box style="margin: 0.35em 0; padding: 0.35em 0.7em;"}
...
:::
::: {.highlight-box style="margin: 0.35em 0; padding: 0.35em 0.7em;"}
**③ Normalise** — zero-mean / unit-variance; prevents one channel dominating
:::
::: {.highlight-box style="margin: 0.35em 0; padding: 0.35em 0.7em;"}
**④ Balance** — SRF faults < 0.1 % of pulses; a naïve model learns to ignore them
:::

::: {.key-result style="margin-top: 0.5em;"}
A model trained on bad data learns bad behaviour. No algorithm compensates for this.
:::
...
raw[55] = 3.9
raw[195] = -3.6
cleaned = (np.sin(t) + 0.1 * np.random.randn(300))
cleaned = (cleaned - cleaned.mean()) / cleaned.std()

fig = make_subplots(
    rows=2, cols=1,
    subplot_titles=["Raw signal — noise & outliers", "After cleaning & normalisation"],
...
for r in [1, 2]:
    fig.update_xaxes(showgrid=False, showticklabels=False, zeroline=False, row=r, col=1)
    fig.update_yaxes(showgrid=True, gridcolor='#1e3048', zeroline=True,
                     zerolinecolor='#334155', row=r, col=1)
...
fig.add_annotation(
    ...
    text="<b style='color:#22d3ee;'>256 numbers → 3 numbers</b>  ·  99 % compression, all information retained",
...
fig.update_layout(
    ...
    paper_bgcolor='#0d1b2a', plot_bgcolor='#111f30',
    font=dict(color='#e2e8f0', size=11),
    legend=dict(bgcolor='rgba(0,0,0,0)', font=dict(size=10), x=0.02, y=0.98),
...
Good features = simpler models + better generalisation + faster training. Domain knowledge beats brute force.
:::
