...
::: {.col-left style="font-size: 0.82em;"}

::: {.highlight-box style="margin-bottom: 0.45em; padding: 0.4em 0.7em;"}
**1990s — First NNs on real machines**

CERN orbit correction · ion source control (SLAC, ALS)
Isolated, hand-crafted — no transfer, no documentation.
:::

::: {.highlight-box style="margin-bottom: 0.45em; padding: 0.4em 0.7em;"}
**2010–2018 — ML enters operations**

SRF fault classification at CEBAF (JLab) · first surrogate models
SVMs and random forests become routine — but still facility-specific.
:::

::: {.highlight-box style="padding: 0.4em 0.7em;"}
**2019 → present — Deep learning changes scale**

RL in production at LCLS & ESRF · differentiable beam dynamics
KIT/KARA: **−30% energy footprint** on a real storage ring (2024)
:::
...
events = [
    (1993, "CERN / ALS<br>first NNs", 1, "#475569"),
    (2016, "Edelen survey<br>revival", -1, "#94a3b8"),
...
for year, label, side, color in events:
    ...
    fig.add_shape(type="line", x0=year, x1=year, y0=0, y1=y_end * 0.82,
        line=dict(color=color, width=1.5, dash="dot"))
    fig.add_trace(go.Scatter(x=[year], y=[0], mode="markers",
        marker=dict(color=color, size=10, line=dict(color="#0d1b2a", width=2)),
        showlegend=False))
    fig.add_annotation(x=year, y=y_end,
...
fig.update_layout(
    height=480, autosize=True, paper_bgcolor="#0d1b2a", plot_bgcolor="#0d1b2a",
...
::: notes
30 years from first experiments to production deployment. KARA −30% energy footprint was measured on the real machine in 2024. The exponential part is now.
:::
...
::: {.key-result style="margin-top: 0.3em; font-size: 0.78em;"}
**This lecture:** Bayesian Optimisation — ~50 evaluations, model-free, works directly on the machine.
:::
...
::: {.key-result style="margin-top: 0.3em; font-size: 0.78em;"}
**This lecture:** MLP — 5 000 Cheetah samples, sub-ms inference, **586× speedup**. Add ensemble for UQ.
:::
